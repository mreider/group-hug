...
p(featured). @curl -O -L http://rvm.beginrescueend.com/src/bootstrap_rails_environment && source ./bootstrap_rails_environment@

* Create rails app:

p(featured). @rails new grouphug@
...
p(featured). @script/rails plugin install git://github.com/mmangino/facebooker2.git@
...
p(featured). @Facebooker2.load_facebooker_yaml@

* Add the following line to @app/controllers/application_controller.rb@

p(featured). @include Facebooker2::Rails::Controller@
...
p(featured). @<% # include JavaScript to use facebook connect%>@ 
@<%= fb_connect_async_js %> @
...
p(featured). @rake db:migrate@
  
* Create the home controller and login action
   
p(featured). @rails generate controller Home@
...
p(featured). @rails generate view home login@
