...
* Put some code into @views/layouts/application.html.erb@ to display the Facebook login button on every page. The conditional logic will replace the login button with the user's name, if he/she is already logged in. Here we are using embedded ruby.
...
* Add some HTML and embedded ruby to this view. This creates a select box of all the groups the user has on Facebook.

p(codey).<pre><select name="group" id="group">
